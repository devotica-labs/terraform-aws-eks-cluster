...
# Devotica fintech guard: if the public API endpoint is opened, it must be
# scoped — never left wide open at 0.0.0.0/0. Advisory (a `check`, not a
# validation) so genuine break-glass changes aren't hard-blocked, but it
# surfaces loudly in every plan/apply until the CIDRs are narrowed.
check "public_endpoint_restricted" {
  assert {
    condition     = !var.endpoint_public_access || !contains(var.public_access_cidrs, "0.0.0.0/0")
    error_message = "endpoint_public_access is true but public_access_cidrs still contains 0.0.0.0/0. Narrow public_access_cidrs to your office/VPN ranges, or keep the API endpoint private (the Devotica default)."
  }
}
